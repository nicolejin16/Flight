...
[WRO Engineering Journal 2026 - Nicole and Summer (PDF)](https://docs.google.com/document/d/1TM94fLH61E81h9KNRlQ1k3E9Kfo4EshCZIIg4DFSXao/edit?pli=1&tab=t.0?raw=true)
...
| Button | [`2 Pin Panel Push Button`](https://www.amazon.ca/dp/B01J9KO7DC?ref=ppx_yo2ov_dt_b_fed_asin_title&th=1) | $12.99 |
...
## Breakdown
This code uses similar parts if the open challenge code, but has had added components to account for the extra pillars during the course, such as obstacle priority, obstacle avoid, and pillar detection. Obstacle priority ensures that the system verifies that a valid pillar bounding box is present and its pixel area exceeds OBSTACLE_TRIGGER_AREA (900 px). It forces the robot to turn to obstacle avoid mode and resets the counter of pixels. Pillar detection makes the camera converts frames to HSV color space and applies threshold masks to isolate red and green pillars, calculating their bounding box dimensions and total pixel area. If both colours are visible, the system prioritizes the larger contour, representing the closer obstacle, and outputs its bounding box, pixel count, and colours label to the state machine. The obstacle challenges uses the same system of counting the orange and blue lines around the course in order to stop in the right place. Since we are not going to be attempting the parallel park at the beginning and end, the robot is going to count to 250 and stop in the middle of the track, similar to the open challenge. 
## Modes
